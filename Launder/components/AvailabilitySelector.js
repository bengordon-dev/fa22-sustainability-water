...
export default function AvailabilitySelector(props) {
  ...
  function pressSector(index) {
    ...
    if (selectedSectors.includes(index)) {
      ...
      for (
        let i = 0;
        intervalIndex < 0 && i < props.freeIntervals.length;
        i++
      ) {
      ...
      if (
        ...
        endTime === newInterval[0] + newInterval[1]
      ) {
        props.setFreeIntervals([
          ...props.freeIntervals.slice(0, intervalIndex),
          ...props.freeIntervals.slice(intervalIndex + 1),
        ]);
      }
      ...
      for (let i = 0; beforeIndex < 0 && i < props.freeIntervals.length; i++) {
        const interval = props.freeIntervals[i];
        if (
          interval[0] <= startTime - 30 &&
          endTime - 30 <= interval[0] + interval[1]
        ) {
          beforeIndex = i;
        }
      }
      ...
      if (beforeIndex == -1 && afterIndex == -1) {
        ...
        props.setFreeIntervals([
          ...props.freeIntervals.slice(0, insertIndex),
          newInterval,
          ...props.freeIntervals.slice(insertIndex),
        ]);
      }
      // adding to the end of an interval (something before)
        ...
        interval[1] += 30;
        props.setFreeIntervals([
          ...props.freeIntervals.slice(0, beforeIndex),
          interval,
          ...props.freeIntervals.slice(beforeIndex + 1),
        ]);
      }
      // adding to the front of an interval (something after)
        ...
        interval[1] += 30;
        props.setFreeIntervals([
          ...props.freeIntervals.slice(0, afterIndex),
          interval,
          ...props.freeIntervals.slice(afterIndex + 1),
        ]);
      }
            ...
            <View style={{ width: "100%", flexDirection: "row" }} key={i}>
              ...
              <TouchableOpacity
                style={{
                  width: "80%",
                  height: 20,
                  backgroundColor: selectedSectors.includes(
                    e + props.nowInterval
                  )
                    ? "#9f9"
                    : "#999",
                ...
                onPress={
                  () => pressSector(e + props.nowInterval) /*{
