...
export default function Graph(props) {
  ...
  useEffect(() => {
    graphData &&
      ...
      setMaxPrice(
        Math.max(
          ...graphData.damSppData.map((e) => e[region]),
          ...graphData.rtSppData.map((e) => e[region])
        )
      );
  }, [graphData]);
...
<<<<<<< HEAD
      ...
      <View width="100%" alignItems="center" justifyContent="center">
          ...
          {points.length > 0 &&
            points.map((point, i) => {
              ...
              return (
                <Circle
                  cx={
                    /*(point.timeStamp-firstTime)/(lastTime-firstTime)*/ (hoursElapsed /
                      24) *
                    150
                  }
        ...
        >
          {[...Array(4).keys()].map((e) => (
            <Text marginLeft={50} color="black">
              {e}
            </Text>
          ))}
        </View>
