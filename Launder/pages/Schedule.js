...
export default function Schedule(props) {
  ...
  function selectTimes(optimizeVal) {
    ...
    const optimizeFunc = (totalPrice, totalRenew) => {
      return optimizeVal === "price"
        ? totalPrice
        : optimizeVal === "renewables"
        ? maxRenewables - totalRenew / (props.washTime + props.dryTime)
        : optimizeVal === "both"
        ? (maxRenewables - totalRenew / (props.washTime + props.dryTime)) *
          totalPrice
        : 0;
    ...
    for (let timeIndex = 0; timeIndex < availability.length; timeIndex++) {
      ...
      for (
        let i = 1;
        i < intervalLength - props.washTime - props.dryTime;
        i++
      ) {
        totalPrice =
          lastPrice +
          priceForMinute(
            startTime + props.washTime + props.dryTime + i,
            props.dryPower
          ) -
